...
> **实现**：`jiuwenclaw/common/e2a/`（`models.py`、`adapters.py`、`constants.py`、`__init__.py`）。**版本**：`E2A_PROTOCOL_VERSION`（默认 `1.0`）。**冲突时**：以 `models.py` 中 dataclass 字段为最终实现源，并回头修正本文。请求：`E2AEnvelope`；响应：`E2AResponse`。  
...
| `jiuwenclaw/common/e2a/models.py` | 请求 `E2AEnvelope`、响应 `E2AResponse` 与子结构；`from_dict` / `to_dict` |
| `jiuwenclaw/common/e2a/constants.py` | **`E2A_SOURCE_PROTOCOL_*`**、**`E2A_RESPONSE_KINDS`**、**`E2A_RESPONSE_STATUS_*`**、ACP 方法名与 SessionUpdate 字符串（运行时以元组为准） |
| `jiuwenclaw/common/e2a/adapters.py` | ACP / A2A → E2A；E2A → ACP JSON-RPC；**`E2AResponse` → ACP / A2A 投影**（见 §8） |
| `jiuwenclaw/common/e2a/__init__.py` | 对外导出 |
...
以下**仅**用于 ACP 桥接或文档对照，**运行时**以 `jiuwenclaw.common.e2a.constants` 中元组为准：
...
*本文与 `jiuwenclaw/common/e2a/models.py`（`E2AEnvelope`、`E2AResponse`）同步维护。*
