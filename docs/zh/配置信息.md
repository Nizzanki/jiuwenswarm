...
此外，面板中还包含 **免费搜索引擎配置**、**多Agent/Team 配置**、**A2UI**、**Agentic 技能检索**、**记忆系统敏感信息过滤**、**邮箱配置** 等区域，按需配置即可。
...
| `model_provider` | 模型提供商类型    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`InferenceAffinity`、`OpenRouter`，用于适配不同服务商的 API 格式；视频/音频/视觉模型当前仅支持 `OpenAI` |
...
| `reasoning_level` | 选填 | 推理强度，可选 `off` / `low` / `medium` / `high`，留空表示不设置 |
...
本节汇总「搜索与外部服务」相关能力的配置。以下配置项均在 **配置信息** 面板中展示（均为可选）。
...
- **自动检测可演进信号**：默认关闭。开启后，系统会在对话和工具执行后自动扫描失败、纠错等演进信号。对应配置 `react.evolution.auto_scan`，环境变量 `EVOLUTION_AUTO_SCAN` 优先。
- **自动建议创建新技能**：默认关闭。开启后，系统可以在缺少合适 Skill 时提出自动创建建议。对应配置 `react.evolution.skill_create`，环境变量 `SKILL_CREATE` 优先。
...
| `react.context_engine_config.round_level_compressor_config.tokens_threshold` | 轮次级压缩 Token 阈值 | `230000` |
