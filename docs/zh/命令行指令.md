...
从 v0.2.1 开始，JiuwenSwarm 提供了原生命令行聊天入口，可以在终端中直接与 JiuwenSwarm 对话。
...
| `--timeout <seconds>` | 300 | 单次 recv 超时（秒） |

### 模式（`--mode`）

支持的模式值与[模式系统](模式系统.md)一致：

| 模式 | 别名 | 说明 |
|---|---|---|
| `code.normal` | `code`、`normal` | 默认，代码普通模式 |
| `code.plan` | — | 代码规划模式 |
| `code.team` | — | 代码团队模式 |
| `agent.plan` | `agent` | Agent 规划模式 |
| `agent.fast` | `fast` | Agent 快速模式 |
| `team` | `team.normal` | 团队模式 |

```bash
# 使用别名
jiuwenswarm chat --mode fast "快速回答"
...
| 操作 | 行为 |
|---|---|
| 第一次 Ctrl+C | 发送 `chat.interrupt` 给 Agent，尝试优雅取消 |
| 第二次 Ctrl+C | 立即退出（`os._exit(130)`） |
