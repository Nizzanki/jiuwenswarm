...
- `/mode`：切换当前通道的工作模式（`plan` / `fast` / `code` / `team`）
...
- 针对受控通道（当前支持：`feishu` / `xiaoyi` / `dingtalk` / `whatsapp` / `wecom` / `wechat`），为该通道生成一个新的 `session_id`，形如：  
  - `feishu_<毫秒时间戳hex>_<随机hex>`
  - `xiaoyi_<毫秒时间戳hex>_<随机hex>`
  - `dingtalk_<毫秒时间戳hex>_<随机hex>`
  - `whatsapp_<毫秒时间戳hex>_<随机hex>`
  - `wecom_<毫秒时间戳hex>_<随机hex>`
  - `wechat_<毫秒时间戳hex>_<随机hex>`
- 后续从该通道进来的所有普通聊天消息，都会被强制覆盖为这个新的 `session_id`，从而在 `workspace/session/` 下形成一个新的会话目录。

**使用方式：**

- 在通道（飞书 / 小艺 / 钉钉 / WhatsApp / 企业微信 / 微信）中，直接发送一条消息：
...
### 2. `/mode` —— 切换通道模式（plan / fast / code / team）

**作用：**

- 为当前通道设置一个逻辑「模式」，用于告诉 Agent 是偏向：
  - `plan`：偏规划、解释、拆解任务（默认，即 `agent.plan`）
  - `fast`：偏自动执行、直接动手（内部与历史 `agent` 模式一致，即 `agent.fast`）
  - `code`：代码模式，Agent 通过代码执行工具与环境交互
    - `code.plan`：代码模式 + 规划风格
    - `code.normal`：代码模式 + 直接执行风格
  - `team`：组队模式
