...
| `/mode code.plan` | 代码模式 + 规划风格 |
| `/mode code.normal` | 代码模式 + 直接执行风格（默认） |
| `/mode code.team` | 代码模式 + 组队风格 |

> 说明：以上是 Gateway 受控通道白名单。TUI 本地 `/mode` 还支持 `/mode plan`（等价于 `agent.plan`）、`/mode team.normal`（等价于 `team`）和 `/mode team.plan`（直达 `team.plan`）；这些写法不会被 Gateway 受控通道识别。
...
4. 回复系统提示，例如：`[收到 CLI 指令], mode 已变更为 code.plan`
