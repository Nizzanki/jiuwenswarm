...
**典型**：`/new_session`、受控通道上的 `/mode agent|code|team` 与 `/switch plan|fast|normal|team`，以及 `/mode agent.plan|agent.fast|code.plan|code.normal|code.team` 直达写法（以当前实现为准）。TUI 本地额外支持 `/mode plan`、`/mode team.normal`、`/mode team.plan`，这些不属于 Gateway 受控通道白名单。
