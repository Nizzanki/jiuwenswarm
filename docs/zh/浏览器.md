...
## 1. 简介
浏览器工具是 JiuwenClaw 中用于驱动真实浏览器完成网页任务的一套能力。它适合处理需要表单填写、页面点击、文件上传、网页检索等场景。
...
### 前端
- `jiuwenclaw/web/src/components/BrowserPanel/index.tsx`
  - 前端浏览器服务面板
...
### Chrome 启动脚本
- `jiuwenclaw/agentserver/tools/browser_start_client.py`
  - 读取 `config/config.yaml` 中的 `browser.*` 配置
  - 启动带远程调试能力的 Chrome

### 浏览器 MCP 接入
- `jiuwenclaw/agentserver/tools/browser_tools.py`
  - 浏览器 MCP wrapper 的接入、自动拉起、client patch、配置构建

### 浏览器运行时
- `jiuwenclaw/agentserver/tools/browser-move/src/playwright_runtime_mcp_server.py`
  - 浏览器 runtime 的 MCP server 入口
- `jiuwenclaw/agentserver/tools/browser-move/src/playwright_runtime/runtime.py`
  - 浏览器 runtime 编排层
- `jiuwenclaw/agentserver/tools/browser-move/src/playwright_runtime/service.py`
  - 浏览器任务执行、session 复用、timeout guardrails
- `jiuwenclaw/agentserver/tools/browser-move/src/playwright_runtime/agents.py`
  - 主编排 agent 与浏览器 worker agent 的提示词和构造逻辑
- `jiuwenclaw/agentserver/tools/browser-move/src/playwright_runtime/config.py`
