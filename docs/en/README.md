# User guide

This section collects common usage notes and feature documentation for JiuwenClaw.

- Installation: [Install guide](InstallGuide.md)
- Install and run from scratch: [Quick start](Quickstart.md) / [TUI mode](Quickstart_tui.md)
- Web UI layout and main areas: [Page overview](Page-Overview.md)（[简体中文版](../zh/页面概览.md)）
- Unified Gateway↔Agent request envelope (E2A): [E2A protocol](E2A-protocol.md)（[中文版](../zh/E2A-protocol.md)）
- A2A protocol integration: [A2A Integration Guide](A2A.md)（[中文版](../zh/A2A.md)）
- ACP client configuration: [ACP Client Config](ACP_Client_Config.md)
- Model and system configuration: [Configuration](Configuration.md)

Topic guides:

- [Channels](Channels.md) — Feishu, Discord, WhatsApp, Web, etc.
- [Modes](Modes.md) — PLAN / AGENT / CODE / TEAM mode switching and configuration
- [CLI commands](CLI.md)
- [Heartbeat](Heartbeat.md)
- [Scheduled tasks](ScheduledTasks.md)
- [Task planning](TaskPlanning.md)
- [Memory](Memory.md)
- [Task Memory](TaskMemory.md) — Task-level experience retrieval and consolidation
- [Context compression & offload](ContextCompression.md)
- [Coding Memory](CodingMemory.md) — Code-mode-specific memory
- [Skills](Skills.md)
- [Skill self-evolution](SkillSelfEvolution.md)
- [Agent & workspace](Agent.md)
- [Browser tools](Browser.md)
- [MCP Configuration](MCPConfiguration.md) — MCP server integration and configuration
- [Tool permissions & security](ToolPermissionsSecurity.md)
- [Slash command architecture](SlashCommandArchitecture.md)
- [Slash command reference](SlashCommands.md)
- [Distributed Team](DistributedTeam.md)（[中文版](../zh/分布式Team.md)）
- [Multi-instance operation](MultiInstance.md)（[中文版](../zh/单机多实例运行.md)）
- [Packaging desktop executables](PackExeGuide.md)
- [Windows auto-update design](WindowsAutoUpdateDesign.md)

Development practices:

- [JiuwenClaw Code Review Assistant](development-practices/JiuwenClaw-Code-Review-Assistant.md)
- [JiuwenClaw Daily Report Generator](development-practices/JiuwenClaw-Daily-Report-Generator.md)