...
| Team (Plan) | `team.plan` | Team collaboration with planning-oriented team runtime settings |
...
/mode team.plan      # TUI local form, directly sets Team Plan mode
```

> Compatibility: `/mode plan`, `/mode team.normal`, and `/mode team.plan` are TUI-local command forms. Gateway controlled channels accept only `agent`, `code`, `team`, `agent.plan`, `agent.fast`, `code.plan`, `code.normal`, and `code.team`.
...
> The TUI source contains a `/switch` implementation, but the default TUI command registry does not currently register it. In TUI, prefer `/mode ...` for direct switching or `/plan` to enter the current family’s plan sub-mode.
...
| `code.plan` | Code Adapter (`mode=code`, `sub_mode=plan`) | Planning phase for coding work. Useful for reading the project, decomposing edits, and preparing an implementation path. | Uses the Code-specific English system prompt; fixed Rails include `LspRail`, `ProjectMemoryRail`, `CodingMemoryRail`, `AgentModeRail`, `StructuredAskUserRail`, `ConfirmInterruptRail`, filesystem/permission Rails; dynamic Rails/tools come from `modes.code.rails` / `modes.code.tools`. | Does not use the general `MemoryRail`; uses `CodingMemoryRail` and project memory files such as `JIUWENSWARM.md` / `CLAUDE.md`. |
| `code.normal` | Code Adapter (`mode=code`, `sub_mode=normal`) | Execution phase for coding work. Useful for editing files, running commands, verifying changes, and delivering results. | Mostly the same Code Rails/tools as `code.plan`; the difference is the current mode semantics and command flow. Some commands, such as `/init`, auto-switch from `code.plan` to `code.normal` before writing. | Same as `code.plan`: coding memory + project memory. |
...
| `team.plan` | Team Plan (canonicalized as `team.plan`, runtime uses the code/team path) | Planning-enhanced Team entry. Intended to give the Team stronger planning semantics, especially for leader coordination. | Sets `enable_team_plan`; the leader can keep the configured runtime language instead of being forced into the Code profile’s English runtime prompt; members still follow the Team / Code-Team inheritance path. | Same Team memory config; Team Plan changes coordination prompts rather than defining a separate memory block. |
...
- `code.plan` and `code.normal` use the same Code Adapter. The difference is mostly mode semantics and command flow; both have the Code prompt, LSP, project memory, coding memory, and file/permission guardrails.
...
- `team.plan` is a planning-enhanced Team entry. The current implementation starts it through the code/team path and enables `enable_team_plan` in TeamManager.
