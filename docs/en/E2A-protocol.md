...
> **Implementation**: `jiuwenclaw/common/e2a/` (`models.py`, `adapters.py`, `constants.py`, `__init__.py`). **Version**: `E2A_PROTOCOL_VERSION` (default `1.0`). **On conflict**: treat the dataclass fields in `models.py` as the source of truth and update this document accordingly. Requests: `E2AEnvelope`; responses: `E2AResponse`.  
...
| `jiuwenclaw/common/e2a/models.py` | Request `E2AEnvelope`, response `E2AResponse`, nested types; `from_dict` / `to_dict` |
| `jiuwenclaw/common/e2a/constants.py` | **`E2A_SOURCE_PROTOCOL_*`**, **`E2A_RESPONSE_KINDS`**, **`E2A_RESPONSE_STATUS_*`**, ACP method names and SessionUpdate strings (runtime: use tuples in code) |
| `jiuwenclaw/common/e2a/adapters.py` | ACP / A2A → E2A; E2A → ACP JSON-RPC; **`E2AResponse` → ACP / A2A projections** (§8) |
| `jiuwenclaw/common/e2a/__init__.py` | Public exports |
...
For ACP bridging and doc cross-check only; **at runtime** use tuples in `jiuwenclaw.common.e2a.constants`:
...
Python: `jiuwenclaw.common.e2a.merge_params_to_acp_prompt(envelope)`.
...
*Maintained in sync with `jiuwenclaw/common/e2a/models.py` (`E2AEnvelope`, `E2AResponse`).*
