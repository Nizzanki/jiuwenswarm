...
def _dispatch() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1].lower() == "acp":
        from jiuwenclaw.channels.acp.app_acp import main as acp_main
        acp_main()
        return
    ...
    if _pop_flag("--desktop-run-web"):
        from jiuwenclaw.channels.web.app_web import main as web_main
        web_main()
        return
    if _pop_flag(_DESKTOP_RUN_AGENT):
        from jiuwenclaw.server.app_agentserver import main as agent_main
        agent_main()
        return
    if _pop_flag(_DESKTOP_RUN_GATEWAY):
        from jiuwenclaw.gateway.app_gateway import main as gateway_main
        gateway_main()
        return
    ...
    from jiuwenclaw.channels.desktop.desktop_app import main as desktop_main
    try:
        desktop_main()
    finally:
